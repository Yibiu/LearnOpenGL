...
bool CGLCylinder::init()
{
	...
	// Top
	_vertices_top.clear();
	_vertices_top.push_back(0.0f);
	_vertices_top.push_back(0.5f);
	_vertices_top.push_back(0.0f);
	for (int i = 0; i <= split; i++) {
		_vertices_top.push_back(r * cos(glm::radians(i * split_rad)));
		_vertices_top.push_back(0.5f);
		_vertices_top.push_back(-r * sin(glm::radians(i * split_rad)));
	}
	glGenVertexArrays(1, &_VAO_top);
	glBindVertexArray(_VAO_top);
	glGenBuffers(1, &_VBO_top);
	glBindBuffer(GL_ARRAY_BUFFER, _VBO_top);
	glBufferData(GL_ARRAY_BUFFER, _vertices_top.size() * sizeof(float), &_vertices_top[0], GL_STATIC_DRAW);
...
void CGLCylinder::uninit()
{
	if (_VBO_top > 0) {
		glDeleteBuffers(1, &_VBO_top);
		_VBO_top = 0;
	}
	if (_VAO_top > 0) {
		glDeleteVertexArrays(1, &_VAO_top);
		_VAO_top = 0;
	}
	_vertices_top.clear();
...
bool CGLCylinder::draw()
{
	if (_VAO_top > 0 && _VAO_side > 0 && _VAO_bottom > 0) {
		glBindVertexArray(_VAO_top);
		glDrawArrays(GL_TRIANGLE_FAN, 0, _vertices_top.size() / 3);

		glBindVertexArray(_VAO_side);
		glDrawArrays(GL_TRIANGLE_STRIP, 0, _vertices_side.size() / 3);
